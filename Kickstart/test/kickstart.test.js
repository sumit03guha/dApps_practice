...
beforeEach(async () => {
    ...
    factory = await new web3.eth.Contract(compiled_contract['FactoryKickstarter'].abi)
                .deploy({ data : compiled_contract['FactoryKickstarter'].evm.bytecode.object})
    ...
    kickstarter = await new web3.eth.Contract(compiled_contract['Kickstarter'].abi, kickstarterAddress);
});
...
describe('Kickstarter' , () => { 
    ...
    it('checks requests' , async () => {
        await kickstarter.methods.createRequest('Buy battery' , '59' , accounts[5])
        .send({ from : accounts[0] , gas : 2000000});
